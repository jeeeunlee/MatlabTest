function [Fc] = ContactForceSwingGivenRatio(robot, pcom_goal, T2, T3, ratio, alpha, beta, pdir)
%% alpha = ratio * beta
global m g
...
Fc=[];
for t=0:0.01:T2
    fcc=( 0.5*m*(T2-t)^2*invA1*skew(g) + m*invA2 )*alpha*pdir + ( 0.5*m*T3*(T3+2*T2-t)*invA1*skew(g) )*beta*pdir +invA*C;
    Fc = [Fc,  fcc];
end
